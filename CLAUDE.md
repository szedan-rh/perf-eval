...
2. **Trigger the build** via `mcp__claude_ai_Buildkite__create_build`:
   ...
   - `environment`: always pass both `VLLM_COMMIT` (the vLLM SHA being tested) and `VLLM_IMAGE` (the full Docker image URI). Optionally pass `WORKLOADS` as a **comma-separated** list of workload stems (e.g. `"qwen3_5_h200,deepseek_v3_2_b200"`); omit it to run all `nightly: true` workloads. Do NOT use spaces to separate entries — the parser only splits on commas and newlines.
